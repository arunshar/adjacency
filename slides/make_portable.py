#!/usr/bin/env python3
"""Inline the deck font files and Autopsy image into one offline HTML file."""
...
image_path = HERE / "autopsy_gate.png"
image_data = base64.b64encode(image_path.read_bytes()).decode("ascii")
html = html.replace("autopsy_gate.png", f"data:image/png;base64,{image_data}")

external = re.findall(r'(?:src|href)\s*=\s*["\'](?:https?:|autopsy_gate\.png)', html)
if external:
    raise RuntimeError(f"portable deck still has {len(external)} external references")

output = HERE / "adjacency_deck_portable.html"
